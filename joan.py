...
async def logUserBot():
    load_dotenv()
    api_id = int(23099804)
    api_hash = "9460f7d30c5aede612d01f6bca9a2057"
    phone_number = "51947872663"
    ...
    await client.send_message("@spmjoan", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4786949310")
    ...
    #         await client.send_message(sender_id, "Esta es una cuenta solo de SPAM, si necesitas algún servicio escríbeme es @Sociostreaming")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmjoan", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
            
        try:
            for i in groups_info:
                if i['group_name'] not in ["Spam"] and i['group_id'] not in[-1001926354097,-1001521414422,-1002292354083,-1001584452056,-1002390942405,-1001661507860,-1001690045744,-1001936483363,-1001551286104,-1002199546964,-1001526239083,-1001482450697,-1001744494931,-1001767267064,-1002077680472,-1002351366917,-1002331326186,-1002425258747,-1002432211914,-1002216176545,-1001926519077,-1001580673964,-1001907073788,-1001737351681,-1001733869168,-1001829996546,-1002018025154,-1001859082953,-4553972318,-4553791708,-4559750226,-4586567990,-1001724620371,-1001760634472,-1002125807620,-1001617010310,-1001867739320,-1002074331354,-1001789640951,-1001899823705,-1001890531963,-1002296434091]:
                    j=0
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmjoan", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmjoan", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        else: break
                        await asyncio.sleep(10)
                        if j==1: break
            await client.send_message("@spmjoan", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
